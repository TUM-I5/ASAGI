...
public:
	...
	void testGetBlock(void)
	{
		...
		for (unsigned int i = 0; i < std::min(WIDTH-offsets[0], sizes[0]); i++)
			for (unsigned int j = 0; j < std::min(LENGTH-offsets[1], sizes[1]); j++)
				for (unsigned int k = 0; k < std::min(HEIGHT-offsets[2], sizes[2]); k++)
		...
		for (unsigned int i = 0; i < std::min(WIDTH-offsets2[0], sizes[0]); i++)
			for (unsigned int j = 0; j < std::min(LENGTH-offsets2[1], sizes[1]); j++)
				for (unsigned int k = 0; k < std::min(HEIGHT-offsets2[2], sizes[2]); k++)
		...
		for (unsigned int i = 0; i < std::min(WIDTH-offsets3[0], sizes[0]); i++)
			for (unsigned int j = 0; j < std::min(LENGTH-offsets3[1], sizes[1]); j++)
				for (unsigned int k = 0; k < std::min(HEIGHT-offsets3[2], sizes[2]); k++)
		...
		for (unsigned int i = 0; i < std::min(WIDTH-offsets4[0], sizes[0]); i++)
			for (unsigned int j = 0; j < std::min(LENGTH-offsets4[1], sizes[1]); j++)
				for (unsigned int k = 0; k < std::min(HEIGHT-offsets4[2], sizes[2]); k++)
		...
		for (unsigned int i = 0; i < std::min(WIDTH-offsets5[0], sizes2[0]); i++)
			for (unsigned int j = 0; j < std::min(LENGTH-offsets5[1], sizes2[1]); j++)
				for (unsigned int k = 0; k < std::min(HEIGHT-offsets5[2], sizes2[2]); k++)
...
private:
	 ...
	 */
	float calcValueAt(int x, int y, int z, size_t offsets[3])
	{
