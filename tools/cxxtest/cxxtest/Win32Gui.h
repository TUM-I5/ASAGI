...
class Win32Gui : public GuiListener {
  ...
  HWND createWindow(LPCTSTR className, DWORD style, HWND parent = (HWND)NULL) {
    return CreateWindow(
        className, NULL, style, 0, 0, 0, 0, parent, (HMENU)NULL, (HINSTANCE)NULL, (LPVOID)this);
  }
  ...
  void setCaption(const char* a = "", const char* b = "", const char* c = "", const char* d = "") {
    unsigned length =
        lstrlenA(_title) + sizeof(" - ") + lstrlenA(a) + lstrlenA(b) + lstrlenA(c) + lstrlenA(d);
  ...
  void setStatusPart(unsigned part, const char* text) { statusBarMessage(SB_SETTEXTA, part, text); }
  ...
  void setSummaryStatusBar() {
    ...
    if (tracker().failedTests())
      wsprintfA(
          _statusTestsDone, "Failed %u of %s %s", tracker().failedTests(), _strTotalTests, tests);
    else
