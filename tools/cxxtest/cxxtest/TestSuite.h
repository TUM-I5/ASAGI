...
void doAssertDelta(const char* file,
  ...
  if (!delta(x, y, d)) {
    ...
    tracker().failedAssertDelta(
        file, line, xExpr, yExpr, dExpr, TS_AS_STRING(x), TS_AS_STRING(y), TS_AS_STRING(d));
    TS_ABORT();
  }
}
...
#define CXXTEST_COMPARISONS(CXXTEST_X, CXXTEST_Y, CXXTEST_T)                                       \
  ...
  inline bool lessThanEquals(CXXTEST_X x, CXXTEST_Y y) {                                           \
    return (((CXXTEST_T)x) <= ((CXXTEST_T)y));                                                     \
  }                                                                                                \
  inline bool lessThanEquals(CXXTEST_Y y, CXXTEST_X x) {                                           \
    return (((CXXTEST_T)y) <= ((CXXTEST_T)x));                                                     \
  }
