 ...
 *
 * SPDX-FileCopyrightText: 2012-2016 Sebastian Rettenberger <rettenbs@in.tum.de>
 */
...
class NetCdfReader {
  ...
  void getBlock(void* block, const size_t* offset, const size_t* size) {
    ...
    std::lock_guard<threads::Mutex> lock(netcdfLock);
    getVar(offset, actSize, size, imap, static_cast<T*>(block));
  }

  /**
   * @return The size of one cell in bytes
   */
  unsigned int getVarSize() const { return m_varSize; }
   ...
   */
  template <typename T>
  void getVar(const size_t* offset,
              const size_t* size,
 ...
 */
template <>
inline void NetCdfReader::getVar(const size_t* offset,
                                 const size_t* size,
                                 const size_t* blockSize,
                                 const ptrdiff_t* imap,
...
inline void NetCdfReader::getVar(const size_t* offset,
  ...
  for (indexFile--; indexFile >= 0 && indexFile < static_cast<long>(indexBuf); indexFile--) {
    memcpy(&byteBuf[indexBuf * getVarSize()], &byteBuf[indexFile * getVarSize()], getVarSize());
